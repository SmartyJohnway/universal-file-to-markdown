...
`VERSION` is the canonical current skill-version source. `1.8.0` is the current development version and `1.7.2` is the latest published stable skill version. `v1.7.1` was merged as an unpublished integration milestone (no `v1.7.1` tag or GitHub Release was published) and was superseded by `v1.7.2`.
