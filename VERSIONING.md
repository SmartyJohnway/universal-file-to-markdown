...
`VERSION` is the canonical current skill-version source. `1.7.2` is the current stable skill version. `v1.7.1` was merged as an unpublished integration milestone (no `v1.7.1` tag or GitHub Release was published). `v1.7.2` supersedes `v1.7.1`, and `1.7.0` remains the previous published stable release.
